...
pack(lua_State *L, void *data, size_t size) {
	struct mc_package * pack = uboss_malloc(sizeof(struct mc_package));
	pack->reference = 0;
	pack->size = (uint32_t)size;
	pack->data = data;
	struct mc_package ** ret = uboss_malloc(sizeof(*ret));
	*ret = pack;
