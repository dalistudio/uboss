...
static int
lunpack(lua_State *L) {
	struct uboss_socket_message *message = lua_touserdata(L,1);
	...
	if (message->type == UBOSS_SOCKET_TYPE_UDP) {
		int addrsz = 0;
		const char * addrstring = uboss_socket_udp_address(message, &addrsz);
		if (addrstring) {
...
lconnect(lua_State *L) {
	...
	struct uboss_context * ctx = lua_touserdata(L, lua_upvalueindex(1));
	int id = uboss_socket_connect(ctx, host, port);
...
llisten(lua_State *L) {
	...
	struct uboss_context * ctx = lua_touserdata(L, lua_upvalueindex(1));
	int id = uboss_socket_listen(ctx, host,port,backlog);
	if (id < 0) {
...
lsend(lua_State *L) {
	...
	int err = uboss_socket_send(ctx, id, buffer, sz);
...
lbind(lua_State *L) {
	...
	int id = uboss_socket_bind(ctx,fd);
	lua_pushinteger(L,id);
	return 1;
}
...
ludp(lua_State *L) {
	...
	int id = uboss_socket_udp(ctx, host, port);
	if (id < 0) {
...
ludp_connect(lua_State *L) {
	...
	if (uboss_socket_udp_connect(ctx, id, host, port)) {
...
ludp_send(lua_State *L) {
	...
	int err = uboss_socket_udp_send(ctx, id, address, buffer, sz);
