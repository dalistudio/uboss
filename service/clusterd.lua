...
local config_name = uboss.getenv "cluster"
local node_address = {}
local node_session = {}
local command = {}
...
function command.listen(source, addr, port)
	local gate = uboss.newservice("gate")
	if port == nil then
		addr, port = string.match(node_address[addr], "([^:]+):(.*)$")
	end
	uboss.call(gate, "lua", "open", { address = addr, port = port })
	uboss.ret(uboss.pack(nil))
end
...
function command.req(...)
	...
	if ok then
		if type(msg) == "table" then
			uboss.ret(cluster.concat(msg))
		else
			uboss.ret(msg)
		end
	else
		uboss.error(msg)
		uboss.response()(false)
	end
end
...
function command.proxy(source, node, name)
	...
	if proxy[fullname] == nil then
		proxy[fullname] = uboss.newservice("clusterproxy", node, name)
	end
	uboss.ret(uboss.pack(proxy[fullname]))
end
...
function command.register(source, name, addr)
	...
	uboss.ret(nil)
	uboss.error(string.format("Register [%s] :%08x", name, addr))
end
...
function command.socket(source, subcmd, fd, msg)
	if subcmd == "data" then
		...
		local ok, response
		if addr == 0 then
			local name = uboss.unpack(msg, sz)
			local addr = register_name[name]
			if addr then
				ok = true
				msg, sz = uboss.pack(addr)
			else
				ok = false
				msg = "name not found"
			end
		else
			ok , msg, sz = pcall(uboss.rawcall, addr, "lua", msg, sz)
		end
		if ok then
	...
	elseif subcmd == "open" then
		uboss.error(string.format("socket accept from %s", msg))
		uboss.call(source, "lua", "accept", fd)
	else
		large_request = {}
		uboss.error(string.format("socket %s %d : %s", subcmd, fd, msg))
	end
end

uboss.start(function()
	loadconfig()
	uboss.dispatch("lua", function(session , source, cmd, ...)
