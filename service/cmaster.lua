...
local function dispatch_slave(fd)
	...
	elseif t == 'Q' then
		...
		uboss.error("Invalid slave message type " .. t)
	end
end
...
local function monitor_slave(slave_id, slave_address)
	...
	uboss.error("slave " ..slave_id .. " is down")
...
uboss.start(function()
	...
	socket.start(fd , function(id, addr)
		uboss.error("connect from " .. addr .. " " .. id)
		socket.start(id)
		local ok, slave, slave_addr = pcall(handshake, id)
		if ok then
			uboss.fork(monitor_slave, slave, slave_addr)
		else
			uboss.error(string.format("disconnect fd = %d, error = %s", id, slave))
			socket.close(id)
		end
	end)
end)
