...
local table = table
local cache = require "uboss.codecache"
...
local function collectobj()
	while true do
		uboss.sleep(600 * 100)	-- sleep 10 min
		collectgarbage()
...
function CMD.new(name, t, ...)
	...
	elseif dt == "string" then
		...
		local _, ret = assert(uboss.pcall(f, ...))
...
function CMD.monitor(name, obj)
	...
	table.insert(v.watch, uboss.response())

	return NORET
end

uboss.start(function()
	uboss.fork(collectobj)
