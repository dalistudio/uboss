...
handle_exit(struct uboss_context * context, uint32_t handle) {
	if (handle == 0) {
		...
		uboss_error(context, "KILL self");
	} else {
		uboss_error(context, "KILL :%0x", handle);
	}
	if (G_NODE.monitor_exit) {
		uboss_send(context,  handle, G_NODE.monitor_exit, PTYPE_CLIENT, 0, NULL, 0);
	}
	uboss_handle_retire(handle);
}
...
cmd_reg(struct uboss_context * context, const char * param) {
	if (param == NULL || param[0] == '\0') {
		...
		return uboss_handle_namehandle(context->handle, param + 1);
	} else {
...
cmd_query(struct uboss_context * context, const char * param) {
	if (param[0] == '.') {
		uint32_t handle = uboss_handle_findname(param+1);
		if (handle) {
...
cmd_name(struct uboss_context * context, const char * param) {
	...
	if (name[0] == '.') {
		return uboss_handle_namehandle(handle_id, name + 1);
	} else {
...
tohandle(struct uboss_context * context, const char * param) {
	...
	if (param[0] == ':') {
		...
		handle = uboss_handle_findname(param+1);
	} else {
...
cmd_launch(struct uboss_context * context, const char * param) {
	...
	args = strsep(&args, "\r\n");
	struct uboss_context * inst = uboss_context_new(mod,args);
	if (inst == NULL) {
		return NULL;
	} else {
...
cmd_logon(struct uboss_context * context, const char * param) {
	...
	if (lastf == NULL) {
		f = uboss_log_open(context, handle);
		if (f) {
...
cmd_logoff(struct uboss_context * context, const char * param) {
	...
	if (f) {
		...
		if (ATOM_CAS_POINTER(&ctx->logfile, f, NULL)) {
			uboss_log_close(context, f, handle);
		}
	}
	uboss_context_release(ctx);
	return NULL;
