...
uboss_handle_register(struct uboss_context *ctx) {
	...
	for (;;) {
		...
		struct uboss_context ** new_slot = uboss_malloc(s->slot_size * 2 * sizeof(struct uboss_context *));
		memset(new_slot, 0, s->slot_size * 2 * sizeof(struct uboss_context *));
		for (i=0;i<s->slot_size;i++) {
...
uboss_handle_retire(uint32_t handle) {
	...
	struct uboss_context * ctx = s->slot[hash];
...
uboss_handle_retireall() {
	...
	for (;;) {
		...
		for (i=0;i<s->slot_size;i++) {
			...
			uint32_t handle = 0;
			if (ctx)
				handle = uboss_context_handle(ctx);
			rwlock_runlock(&s->lock);
			if (handle != 0) {
				if (uboss_handle_retire(handle)) {
					++n;
				}
			}
		}
...
_insert_name_before(struct handle_storage *s, char *name, uint32_t handle, int before) {
	if (s->name_count >= s->name_cap) {
		...
		struct handle_name * n = uboss_malloc(s->name_cap * sizeof(struct handle_name));
		int i;
		for (i=0;i<before;i++) {
			n[i] = s->name[i];
		}
...
uboss_handle_init(int harbor) {
	assert(H==NULL);
	struct handle_storage * s = uboss_malloc(sizeof(*H));
	s->slot_size = DEFAULT_SLOT_SIZE;
	s->slot = uboss_malloc(s->slot_size * sizeof(struct uboss_context *));
	memset(s->slot, 0, s->slot_size * sizeof(struct uboss_context *));
