...
fill_prefix(char* ptr) {
	uint32_t handle = uboss_current_handle();
...
mallctl_int64(const char* name, size_t* newval) {
	...
	// uboss_error(NULL, "name: %s, value: %zd\n", name, v);
	return v;
}
...
mallctl_opt(const char* name, int* newval) {
	...
	if(newval) {
		...
		if(ret == 0) {
			uboss_error(NULL, "set new value(%d) for (%s) succeed\n", *newval, name);
		} else {
			uboss_error(NULL, "set new value(%d) for (%s) failed: error -> %d\n", *newval, name, ret);
		}
	} else {
		je_mallctl(name, &v, &len, NULL, 0);
	}
...
void *
uboss_realloc(void *ptr, size_t size) {
	if (ptr == NULL) return uboss_malloc(size);
...
void *
uboss_calloc(size_t nmemb,size_t size) {
...
void 
memory_info_dump(void) {
	uboss_error(NULL, "No jemalloc");
}

size_t 
mallctl_int64(const char* name, size_t* newval) {
	uboss_error(NULL, "No jemalloc : mallctl_int64 %s.", name);
	return 0;
}

int 
mallctl_opt(const char* name, int* newval) {
	uboss_error(NULL, "No jemalloc : mallctl_opt %s.", name);
	return 0;
}
...
dump_c_mem() {
	int i;
	size_t total = 0;
	uboss_error(NULL, "dump all service mem:");
	for(i=0; i<SLOT_SIZE; i++) {
		mem_data* data = &mem_stats[i];
		if(data->handle != 0 && data->allocated != 0) {
			total += data->allocated;
			uboss_error(NULL, "0x%x -> %zdkb", data->handle, data->allocated >> 10);
		}
	}
	uboss_error(NULL, "+total: %zdkb",total >> 10);
}
...
void
uboss_debug_memory(const char *info) {
	// for debug use
	uint32_t handle = uboss_current_handle();
