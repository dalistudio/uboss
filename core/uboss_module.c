...
struct modules {
	...
	const char * path;
	struct uboss_module m[MAX_MODULE_TYPE];
};
...
uboss_module_query(const char * name) {
	struct uboss_module * result = _query(name);
	if (result)
		return result;
...
uboss_module_insert(struct uboss_module *mod) {
	...
	struct uboss_module * m = _query(mod->name);
...
uboss_module_init(const char *path) {
	struct modules *m = uboss_malloc(sizeof(*m));
	m->count = 0;
	m->path = uboss_strdup(path);
