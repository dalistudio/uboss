...
local uboss_core = require "uboss.core"
local assert = assert
...
local function suspend(s)
	...
	if s.closing then
		uboss.wakeup(s.closing)
	end
end

-- read uboss_socket.h for these macro
-- SKYNET_SOCKET_TYPE_DATA = 1
socket_message[1] = function(id, size, data)
	local s = socket_pool[id]
	if s == nil then
		uboss.error("socket: drop package from " .. id)
	...
	if rrt == "number" then
		...
		if s.buffer_limit and sz > s.buffer_limit then
			uboss.error(string.format("socket buffer overflow: fd=%d size=%d", id , sz))
...
socket_message[5] = function(id, _, err)
	local s = socket_pool[id]
	if s == nil then
		uboss.error("socket: error on unknown", id, err)
		return
	end
	if s.connected then
		uboss.error("socket: error on", id, err)
...
socket_message[6] = function(id, size, data, address)
	...
	if s == nil or s.callback == nil then
		uboss.error("socket: drop udp package from " .. id)
		driver.drop(data, size)
		return
	end
	local str = uboss.tostring(data, size)
	uboss_core.trash(data, size)
...
local function default_warning(id, size)
	local s = socket_pool[id]
		...
		if last + 64 < size then	-- if size increase 64K
			s.warningsize = size
			uboss.error(string.format("WARNING: %d K bytes need to send out (fd = %d)", size, id))
		end
		s.warningsize = size
end
...
uboss.register_protocol {
	name = "socket",
	id = uboss.PTYPE_SOCKET,	-- PTYPE_SOCKET = 6
...
function socket.close(id)
	...
	if s.connected then
		...
		-- because uboss.wait never return in __gc, so driver.clear may not be called
		if s.co then
