...
local function assert_socket(service, v, fd)
	if v then
		...
		uboss.error(string.format("%s failed: socket (fd = %d) closed", service, fd))
		error(socket_error)
	end
end
...
local function launch_slave(auth_handler)
	...
	local function ret_pack(ok, err, ...)
		if ok then
			return uboss.pack(err, ...)
		else
			if err == socket_error then
				return uboss.pack(nil, "socket error")
			else
				return uboss.pack(false, err)
			end
		end
	end

	local function auth_fd(fd, addr)
		uboss.error(string.format("connect from %s (fd = %d)", addr, fd))
	...
	uboss.dispatch("lua", function(_,_,...)
		...
		if ok then
			...
			uboss.ret(uboss.pack(false, msg))
		end
	end)
end
...
local function accept(conf, s, fd, addr)
	-- call slave auth
	local ok, server, uid, secret = uboss.call(s, "lua",  fd, addr)
...
local function launch_master(conf)
	...
	uboss.dispatch("lua", function(_,source,command, ...)
		uboss.ret(uboss.pack(conf.command_handler(command, ...)))
	end)

	for i=1,instance do
		table.insert(slave, uboss.newservice(SERVICE_NAME))
	end

	uboss.error(string.format("login server listen at : %s %d", host, port))
	...
	socket.start(id , function(fd, addr)
		...
		if not ok then
			if err ~= socket_error then
				uboss.error(string.format("invalid client (fd = %d) error = %s", fd, err))
			end
		end
...
local function login(conf)
	...
	uboss.start(function()
		local loginmaster = uboss.localname(name)
		if loginmaster then
