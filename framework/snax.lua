...
local interface_g = uboss.getenv("snax_interface_g")
...
local uboss_send = uboss.send
local uboss_call = uboss.call
...
local function gen_req(type, handle)
	return setmetatable({} , {
		__index = function( t, k )
			...
			return function(...)
				return uboss_call(handle, "snax", id, ...)
			end
		end })
end
...
function snax.rawnewservice(name, ...)
	...
	local handle = uboss.newservice("snaxd", name)
	assert(handle_cache[handle] == nil)
	if t.system.init then
		uboss.call(handle, "snax", t.system.init, ...)
	end
	return handle
end
...
function snax.uniqueservice(name, ...)
	local handle = assert(uboss.call(".service", "lua", "LAUNCH", "snaxd", name, ...))
	return snax.bind(handle, name)
end

function snax.globalservice(name, ...)
	local handle = assert(uboss.call(".service", "lua", "GLAUNCH", "snaxd", name, ...))
	return snax.bind(handle, name)
end

function snax.queryservice(name)
	local handle = assert(uboss.call(".service", "lua", "QUERY", "snaxd", name))
	return snax.bind(handle, name)
end

function snax.queryglobal(name)
	local handle = assert(uboss.call(".service", "lua", "GQUERY", "snaxd", name))
	return snax.bind(handle, name)
end

function snax.kill(obj, ...)
	local t = snax.interface(obj.type)
	uboss_call(obj.handle, "snax", t.system.exit, ...)
end

function snax.self()
	return snax.bind(uboss.self(), SERVICE_NAME)
end
...
function snax.hotfix(obj, source, ...)
	local t = snax.interface(obj.type)
	return test_result(uboss_call(obj.handle, "snax", t.system.hotfix, source, ...))
end

function snax.printf(fmt, ...)
	uboss.error(string.format(fmt, ...))
end
