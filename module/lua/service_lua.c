...
_launch(struct uboss_context * context, void *ud, int type, int session, uint32_t source , const void * msg, size_t sz) {
	...
	struct lua *l = ud;
	uboss_callback(context, NULL, NULL);
	int err = _init(l, context, msg, sz);
	if (err) {
		uboss_command(context, "EXIT", NULL);
	}
...
lua_init(struct lua *l, struct uboss_context *ctx, const char * args) {
	int sz = strlen(args);
	char * tmp = uboss_malloc(sz);
	memcpy(tmp, args, sz);
	uboss_callback(ctx, l , _launch);
	const char * self = uboss_command(ctx, "REG", NULL);
	uint32_t handle_id = strtoul(self+1, NULL, 16);
	// it must be first message
	uboss_send(ctx, 0, handle_id, PTYPE_TAG_DONTCOPY,0, tmp, sz);
	return 0;
}

struct lua *
lua_create(void) {
	struct lua * l = uboss_malloc(sizeof(*l));
	memset(l,0,sizeof(*l));
	l->L = lua_newstate(uboss_lalloc, NULL);
	return l;
}
