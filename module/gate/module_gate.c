...
struct gate *
gate_create(void) {
	struct gate * g = uboss_malloc(sizeof(*g));
	memset(g,0,sizeof(*g));
	g->listen_id = -1;
	return g;
}
...
gate_release(struct gate *g) {
	...
	for (i=0;i<g->max_connection;i++) {
		...
		if (c->id >=0) {
			uboss_socket_close(ctx, c->id);
		}
	}
	if (g->listen_id >= 0) {
		uboss_socket_close(ctx, g->listen_id);
	}
...
_ctrl(struct gate * g, const void * msg, int sz) {
	...
	if (memcmp(command,"broker",i)==0) {
		_parm(tmp, sz, i);
		g->broker = uboss_queryname(ctx, command);
		return;
	}
...
dispatch_socket_message(struct gate *g, const struct uboss_socket_message * message, int sz) {
	...
	case UBOSS_SOCKET_TYPE_DATA: {
		...
		if (id>=0) {
			...
			uboss_socket_close(ctx, message->id);
	...
	case UBOSS_SOCKET_TYPE_ACCEPT:
		...
		if (hashid_full(&g->hash)) {
			uboss_socket_close(ctx, message->ud);
		} else {
...
_cb(struct uboss_context * ctx, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	...
	case PTYPE_CLIENT: {
		...
		if (id>=0) {
			...
			uboss_socket_send(ctx, uid, (void*)msg, sz-4);
...
start_listen(struct gate *g, char * listen_addr) {
	...
	g->listen_id = uboss_socket_listen(ctx, host, port, BACKLOG);
	if (g->listen_id < 0) {
		return 1;
	}
	uboss_socket_start(ctx, g->listen_id);
	return 0;
}
...
gate_init(struct gate *g , struct uboss_context * ctx, char * parm) {
	...
	if (watchdog[0] == '!') {
		...
		g->watchdog = uboss_queryname(ctx, watchdog);
		if (g->watchdog == 0) {
	...
	g->conn = uboss_malloc(max * sizeof(struct connection));
