...
push_queue_msg(struct harbor_msg_queue * queue, struct harbor_msg * m) {
	...
	if (((queue->tail + 1) % queue->size) == queue->head) {
		struct harbor_msg * new_buffer = uboss_malloc(queue->size * 2 * sizeof(struct harbor_msg));
		int i;
		for (i=0;i<queue->size-1;i++) {
			new_buffer[i] = queue->data[(i+queue->head) % queue->size];
		}
		uboss_free(queue->data);
...
new_queue() {
	struct harbor_msg_queue * queue = uboss_malloc(sizeof(*queue));
	queue->size = DEFAULT_QUEUE_SIZE;
	queue->head = 0;
	queue->tail = 0;
	queue->data = uboss_malloc(DEFAULT_QUEUE_SIZE * sizeof(struct harbor_msg));
...
hash_insert(struct hashmap * hash, const char name[GLOBALNAME_LENGTH]) {
	...
	struct keyvalue * node = uboss_malloc(sizeof(*node));
...
hash_new() {
	struct hashmap * h = uboss_malloc(sizeof(struct hashmap));
	memset(h,0,sizeof(*h));
	return h;
}
...
report_harbor_down(struct harbor *h, int id) {
	...
	uboss_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, down, n);
}

struct harbor *
harbor_create(void) {
	struct harbor * h = uboss_malloc(sizeof(*h));
	memset(h,0,sizeof(*h));
	h->map = hash_new();
	return h;
}
...
harbor_release(struct harbor *h) {
	...
	for (i=1;i<REMOTE_MAX;i++) {
		...
		if (s->fd && s->status != STATUS_DOWN) {
			...
			// never call uboss_send during module exit, because of dead lock
		}
	}
	hash_delete(h->map);
	uboss_free(h);
...
forward_local_messsage(struct harbor *h, void *msg, int sz) {
	...
	if (uboss_send(h->ctx, header.source, destination, type | PTYPE_TAG_DONTCOPY , (int)header.session, (void *)msg, sz-HEADER_COOKIE_LENGTH) < 0) {
		if (type != PTYPE_ERROR) {
			// don't need report error when type is error
			uboss_send(h->ctx, destination, header.source , PTYPE_ERROR, (int)header.session, NULL, 0);
		}
		uboss_error(h->ctx, "Unknown destination :%x from :%x type(%d)", destination, header.source, type);
	}
}

static void
send_remote(struct uboss_context * ctx, int fd, const char * buffer, size_t sz, struct remote_message_header * cookie) {
	size_t sz_header = sz+sizeof(*cookie);
	if (sz_header > UINT32_MAX) {
		uboss_error(ctx, "remote message from :%08x to :%08x is too large.", cookie->source, cookie->destination);
		return;
	}
	uint8_t * sendbuf = uboss_malloc(sz_header+4);
	...
	uboss_socket_send(ctx, fd, sendbuf, sz_header+4);
}
...
dispatch_name_queue(struct harbor *h, struct keyvalue * node) {
	...
	struct uboss_context * context = h->ctx;
	...
	if (fd == 0) {
		if (s->status == STATUS_DOWN) {
			...
			uboss_error(context, "Drop message to %s (in harbor %d)",tmp,harbor_id);
		} else {
			if (s->queue == NULL) {
				s->queue = node->queue;
				node->queue = NULL;
			...
			if (harbor_id == (h->slave >> HANDLE_REMOTE_SHIFT)) {
				...
				while ((m = pop_queue(s->queue)) != NULL) {
					...
					uboss_send(context, m->header.source, handle , type | PTYPE_TAG_DONTCOPY, m->header.session, m->buffer, m->size);
				}
				release_queue(s->queue);
				s->queue = NULL;
			}
...
static void
push_socket_data(struct harbor *h, const struct uboss_socket_message * message) {
	assert(message->type == UBOSS_SOCKET_TYPE_DATA);
	int fd = message->id;
	int i;
	int id = 0;
	struct slave * s = NULL;
	...
	if (s == NULL) {
		uboss_free(message->buffer);
		uboss_error(h->ctx, "Invalid socket fd (%d) data", fd);
		return;
	}
	...
	for (;;) {
		...
		case STATUS_HANDSHAKE: {
			...
			if (remote_id != id) {
				uboss_error(h->ctx, "Invalid shakehand id (%d) from fd = %d , harbor = %d", id, fd, remote_id);
				close_harbor(h,id);
				return;
			}
			++buffer;
		...
		case STATUS_HEADER: {
			...
			if (size < need) {
				...
				if (s->size[0] != 0) {
					uboss_error(h->ctx, "Message is too long from harbor %d", id);
					close_harbor(h,id);
					return;
				}
				s->length = s->size[1] << 16 | s->size[2] << 8 | s->size[3];
				s->read = 0;
				s->recv_buffer = uboss_malloc(s->length);
...
remote_send_handle(struct harbor *h, uint32_t source, uint32_t destination, int type, int session, const char * msg, size_t sz) {
	...
	if (harbor_id == h->id) {
		// local message
		uboss_send(context, source, destination , type | PTYPE_TAG_DONTCOPY, session, (void *)msg, sz);
		return 1;
	}
	...
	if (s->fd == 0 || s->status == STATUS_HANDSHAKE) {
		if (s->status == STATUS_DOWN) {
			...
			uboss_send(context, destination, source, PTYPE_ERROR, 0 , NULL, 0);
			uboss_error(context, "Drop message to harbor %d from %x to %x (session = %d, msgsz = %d)",harbor_id, source, destination,session,(int)sz);
		} else {
			if (s->queue == NULL) {
				s->queue = new_queue();
			}
...
remote_send_name(struct harbor *h, uint32_t source, const char name[GLOBALNAME_LENGTH], int type, int session, const char * msg, size_t sz) {
	...
	if (node->value == 0) {
		...
		uboss_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, query, strlen(query));
		return 1;
	} else {
...
handshake(struct harbor *h, int id) {
	struct slave *s = &h->s[id];
	uint8_t * handshake = uboss_malloc(1);
	handshake[0] = (uint8_t)h->id;
	uboss_socket_send(h->ctx, s->fd, handshake, 1);
}
...
harbor_command(struct harbor * h, const char * msg, size_t sz, int session, uint32_t source) {
	...
	case 'N' : {
		if (s <=0 || s>= GLOBALNAME_LENGTH) {
			uboss_error(h->ctx, "Invalid global name %s", name);
			return;
		}
	...
	case 'A' : {
		...
		if (fd == 0 || id <= 0 || id>=REMOTE_MAX) {
			uboss_error(h->ctx, "Invalid command %c %s", msg[0], buffer);
			return;
		}
		struct slave * slave = &h->s[id];
		if (slave->fd != 0) {
			uboss_error(h->ctx, "Harbor %d alreay exist", id);
			return;
		}
		slave->fd = fd;

		uboss_socket_start(h->ctx, fd);
		handshake(h, id);
		if (msg[0] == 'S') {
	...
	default:
		uboss_error(h->ctx, "Unknown command %s", msg);
		return;
	}
}
...
static int
mainloop(struct uboss_context * context, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
	struct harbor * h = ud;
	switch (type) {
	case PTYPE_SOCKET: {
		const struct uboss_socket_message * message = msg;
		switch(message->type) {
		...
		case UBOSS_SOCKET_TYPE_CLOSE: {
			...
			if (id) {
				...
				uboss_error(context, "Unkown fd (%d) closed", message->id);
			}
			break;
		}
		...
		case UBOSS_SOCKET_TYPE_WARNING: {
			int id = harbor_id(h, message->id);
			if (id) {
				uboss_error(context, "message havn't send to Harbor (%d) reach %d K", id, message->ud);
			}
			break;
		}
		default:
			uboss_error(context, "recv invalid socket message type %d", type);
			break;
		}
		return 0;
	}
...
int
harbor_init(struct harbor *h, struct uboss_context *ctx, const char * args) {
	h->ctx = ctx;
	int harbor_id = 0;
	uint32_t slave = 0;
